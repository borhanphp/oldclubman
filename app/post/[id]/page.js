...
import { IoMdShareAlt, IoIosShareAlt } from 'react-icons/io';
...
const PostDetailsPage = () => {
    ...
    const fetchPostDetails = useCallback(async () => {
        try {
            ...
            try {
                ...
                console.warn('Public endpoint failed, trying fallback to client endpoint...', publicErr);
                try {
                    response = await api.get(`/client/singlePost/${params.id}`);
                } catch (clientErr) {
                    console.error('Fallback endpoint also failed:', clientErr);
                    throw clientErr;
                }
            }
    ...
    const handleReaction = (reactionType) => {
        ...
        dispatch(storePostReactions({ post_id: post.id, reaction_type: reactionType }))
            .then(() => fetchPostDetails());
        setShowReactions(false);
    };

    const handleDeleteReaction = () => {
        if (!post) return;
        dispatch(deletePostReaction(post.id)).then(() => fetchPostDetails());
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex flex-col md:flex-row h-screen w-screen bg-black overflow-hidden">
                        ...
                        <div className="flex flex-col gap-3">
                            <div className="flex gap-3">
                                <button
                                    ...
                                    disabled={isSharing}
                                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-white rounded-lg transition-colors ${isSharing ? 'bg-blue-400 cursor-not-allowed' : 'bg-[#1877F2] hover:bg-[#166FE5]'}`}
                                >
                                    Share with Facebook
                                </button>
                                <button
