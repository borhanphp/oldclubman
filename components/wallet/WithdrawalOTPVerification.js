...
import { verifyWithdrawalOTP, resendWithdrawalOTP, getWalletBalance } from '@/views/wallet/store';

const WithdrawalOTPVerification = ({ onSuccess, onCancel }) => {
  const dispatch = useDispatch();
  const { pendingWithdrawal, loading } = useSelector(({ wallet }) => wallet);
  ...
  const handleVerify = async () => {
    ...
    try {
      await dispatch(verifyWithdrawalOTP({
        withdrawal_request_id: pendingWithdrawal.withdrawal_request_id,
        otp_code: otpCode
      })).unwrap();
  ...
  const handleResend = async () => {
    try {
      await dispatch(resendWithdrawalOTP(pendingWithdrawal.withdrawal_request_id)).unwrap();
        ...
        <div className="bg-blue-50 rounded-lg p-4 mb-6">
          <div className="flex justify-between text-sm text-gray-600 mb-1">
            ...
            <span className="font-bold text-blue-600 text-lg">
              ${pendingWithdrawal.amount_dollars}
            </span>
          </div>
          <div className="flex justify-between text-sm text-gray-600 mb-1">
            <span>Fee:</span>
            <span className="font-medium text-gray-800">
              ${pendingWithdrawal.fee_dollars}
            </span>
          </div>
          <div className="flex justify-between text-sm text-gray-600 mb-1 pt-2 border-t border-blue-200">
            <span>Total Deducted:</span>
            <span className="font-bold text-gray-800">
              ${pendingWithdrawal.total_deducted_dollars}
            </span>
          </div>
          {pendingWithdrawal.bank_account_masked && (
            <div className="flex justify-between text-sm text-gray-600 mt-2 pt-2 border-t border-blue-200">
              <span>Account:</span>
              <span className="font-medium text-gray-800">
                {pendingWithdrawal.bank_account_masked}
              </span>
