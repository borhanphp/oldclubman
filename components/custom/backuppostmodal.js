...
const PostModal = () => {
  ...
  const routeMarkersRef = useRef({ origin: null, destination: null });
  ...
  const resetRoute = () => {
    ...
    if (routeMarkersRef.current.destination) {
  ...
  useEffect(() => {
    ...
    loadLeafletAssets().then(() => {
      ...
      const L = window.L;
      const map = L.map(routeMapContainerRef.current).setView([23.8103, 90.4125], 5);
      ...
      map.on('click', (e) => {
        const { latlng } = e;
        if (!routeMarkersRef.current.origin) {
          routeMarkersRef.current.origin = L.marker(latlng, { draggable: true }).addTo(map);
          ...
          routeMarkersRef.current.destination = L.marker(latlng, { draggable: true }).addTo(map);
            ...
            {showRoute && (
              <>
                ...
                <div className="mt-2 text-xs text-gray-600">
                  {!routeOrigin &&
                    !routeDestination &&
                    "Click on the map to set origin, then destination. Drag pins to adjust."}
                  {routeOrigin &&
                    !routeDestination &&
                    "Origin set. Click again to set destination."}
                  {routeOrigin && routeDestination && (
                    <span>
                      From: {routeOrigin.lat.toFixed(5)},{" "}
                      {routeOrigin.lng.toFixed(5)} → To:{" "}
