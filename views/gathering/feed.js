...
import { FaEllipsisH, FaVideo, FaGlobe, FaComment, FaLock } from 'react-icons/fa';
...
const GatheringContent = () => {
  ...
  const handleCommentSubmit = (postId) => {
    ...
    if (!comment) return;
    dispatch(storeComments({post_id: postId, content: comment}))
    .then((res) => {
      dispatch(getGathering());
      dispatch(getPosts());
  ...
  const handleReaction = (postId, reaction) => {
    dispatch(storePostReactions({ post_id: postId, reaction_type: reaction }))
      .then(() => {
  ...
  const handleModalReplySubmit = (commentIndex) => {
    ...
    setModalReplies(prev => ({
      ...prev,
      [commentIndex]: [...(prev[commentIndex] || []), { user: "You", text: reply }]
    }));
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="md:col-span-6">
              ...
              {postsData?.data?.map((item, index) => {
                const totalCount = item.multiple_reaction_counts.reduce((sum, dd) => sum + dd.count, 0);
                console.log('Total count:', totalCount);
                console.log('item', item);
                return(
                  <div className="bg-white rounded-lg shadow-sm p-4 mb-4" key={item.id || index}>
                    ...
                    <div className="flex">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-medium">{item?.client?.fname + " " + item?.client?.last_name}</h4>
                          <span className="text-gray-500">•</span>
                          <p className="text-sm text-gray-500">{moment(item.created_at)?.fromNow()}</p>
                        </div>
                        <p className="text-gray-500 text-sm">
                          This Account Location Not Set Yet. {item?.privacy_mode === "public" ? <FaGlobe className="inline ml-1" /> : <FaLock className="inline ml-1" />} 
                        </p>
                      </div>
                    </div>
                    <div className="relative">
                      <button className="text-gray-500 cursor-pointer" onClick={() => setOpenDropdownFor(openDropdownFor === item.id ? null : item.id)}>
                        <FaEllipsisH />
                      </button>
                      {openDropdownFor === item.id && (
                        <div ref={dropdownRef} className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl z-20">
                          <button onClick={() => handleEditPost(item.id)} className="cursor-pointer flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                            <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536M9 13l6-6m2 2l-6 6m-2 2h2v2a2 2 0 002 2h2a2 2 0 002-2v-2a2 2 0 00-2-2h-2v-2a2 2 0 00-2-2H9a2 2 0 00-2 2v2a2 2 0 002 2z" /></svg>
                            Edit post
                          </button>
                          <button onClick={() => handleOnlyMe(item)} className="cursor-pointer flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                            <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" y2="15" /><line x1="9" y1="9" x2="15" y2="15" /></svg>
                            {item?.privacy_mode === "public" ? "Only Me" : "Public"}
                          </button>
                          <button onClick={() => handleDeletePost(item.id)} className="cursor-pointer flex items-center w-full px-4 py-2 text-sm text-red-600 hover:bg-gray-100">
                  ...
                  <div className="post-content">
                    ...
                    {item?.files && item?.files?.length > 0 && (
                      <div className={`mt-3 grid ${item?.files?.length === 1 ? 'grid-cols-1' : item?.files?.length === 2 ? 'grid-cols-2' : item.files?.length >= 3 ? 'grid-cols-3' : ''} gap-2`}>
                        {item?.files?.map((file, fileIndex) => (
                          <div 
                            key={fileIndex} 
                            className={`overflow-hidden rounded-lg ${item.files?.length === 1 ? 'max-h-96' : 'h-48'} bg-gray-100`}
                          >
                            <img 
                    ...
                    <div className="flex-1 relative">
                      <div className="relative">
                        ...
                        {showReactionsFor === item.id && (
                          ...
                          >
                            <button className="transform hover:scale-125 transition-transform" onClick={() => handleReaction(item.id, 'like')}>
                              <img src="/like.png" alt="Like" className="w-8 h-8" />
                            </button>
                            <button className="transform hover:scale-125 transition-transform" onClick={() => handleReaction(item.id, 'love')}>
                              <img src="/love.png" alt="Love" className="w-8 h-8" />
                            </button>
                            <button className="transform hover:scale-125 transition-transform" onClick={() => handleReaction(item.id, 'care')}>
                              <img src="/care.png" alt="Care" className="w-8 h-8" />
                            </button>
                            <button className="transform hover:scale-125 transition-transform" onClick={() => handleReaction(item.id, 'haha')}>
                              <img src="/haha.png" alt="Haha" className="w-8 h-8" />
                            </button>
                            <button className="transform hover:scale-125 transition-transform" onClick={() => handleReaction(item.id, 'wow')}>
                              <img src="/wow.png" alt="Wow" className="w-8 h-8" />
                            </button>
                            <button className="transform hover:scale-125 transition-transform" onClick={() => handleReaction(item.id, 'sad')}>
  ...
  <div className='cursor-pointer' onClick={() => {handleViewAllComments(item.id)}}>View all comments</div>
                  {/* Comments section */}
                          ...
                          <div className="flex">
                            ...
                            <div className="p-2 rounded-lg flex-grow">
                              <div className="flex flex-col bg-gray-100 p-2 rounded-md">
                                <span className="font-medium">{item.client.fname} <span className="text-xs text-gray-500 ml-2">{moment(item.created_at)?.fromNow()}</span></span>
                                <span className="text-gray-700 text-sm">{item?.latest_comment?.content}</span>
                              </div>
                      ...
                      return (
                        <div className="mb-2" key={i}>
                          <div className="flex">
                            ...
                            <div className="p-2 rounded-lg flex-grow">
                              <div className="flex flex-col bg-gray-100 p-2 rounded-md">
                                <span className="font-medium">{c.user} <span className="text-xs text-gray-500 ml-2">{moment(c.created_at)?.fromNow()}</span></span>
      ...
      {showCommentsModal && basicPostData && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowCommentsModal(false)}>
          <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center justify-between p-6 pb-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-semibold">{basicPostData.client.fname} {basicPostData.client.last_name}</div>
                  <div className="text-xs text-gray-500">{moment(basicPostData.created_at).fromNow()}</div>
                </div>
              </div>
              <button
            ...
            <div className="overflow-y-auto flex-1 px-6 pt-4 pb-2">
              {/* Post message */}
              <div className="text-gray-800 mb-4 break-words">{basicPostData.message}</div>
              {/* Post images if any */}
              {basicPostData.files && basicPostData.files.length > 0 && (
                <div className="rounded-lg overflow-hidden border bg-white mb-4">
                  <img
                    src={process.env.NEXT_PUBLIC_FILE_PATH + "/" + basicPostData.files[0].file_path}
                    alt="Post image"
                    ...
                    <div className="flex-1">
                     <div className='bg-gray-100 p-3 rounded-2xl'>
                      <div className="font-medium text-sm">
                        {c?.client_comment?.fname + " " + c?.client_comment?.last_name} <span className="text-xs text-gray-500 ml-2">{moment(c.created_at).fromNow()}</span>
                      </div>
                      ...
                      <div className="flex gap-3 mt-2 text-xs text-gray-500">
                        ...
                        <button
                          className="hover:underline"
                          onClick={() => setModalReplyInputs(prev => ({ ...prev, [i]: prev[i] === undefined ? '' : prev[i] }))}
                          type="button"
                        >
                          Reply
                        </button>
                      ...
                      {modalReplyInputs[i] !== undefined && (
                        <div className="flex mt-2">
                          <input
                            ...
                            onChange={e => setModalReplyInputs(prev => ({ ...prev, [i]: e.target.value }))}
                          />
                          <button
                            className="ml-2 text-blue-500 text-xs"
            ...
            <div className="p-4 bg-gray-50 flex items-center gap-2">
              ...
              <input
                ...
                onChange={e => setCommentInputs({ ...commentInputs, [basicPostData.id]: e.target.value })}
              ...
              className="ml-2 px-4 py-2 bg-blue-600 text-white rounded-full text-sm font-semibold">Post</button>
            </div>
          </div>
        </div>
      )}
