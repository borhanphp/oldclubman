...
import { getGathering } from './store';



const GatheringContent = () => {
  const {gatheringData, loading} = useSelector(({gathering}) => gathering);
        ...
        <div className="mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div className="md:col-span-6">
              ...
              {/* Post */}
              {gatheringData?.post?.map((item, index) => {
                return(
                  ...
                  <div className="post-content">
                    ...
                    {item?.post_files && item.post_files.length > 0 && (
                      <div className={`mt-3 grid ${item.post_files.length === 1 ? 'grid-cols-1' : item.post_files.length === 2 ? 'grid-cols-2' : item.post_files.length >= 3 ? 'grid-cols-3' : ''} gap-2`}>
                        {item.post_files.map((file, fileIndex) => (
                          <div 
                            key={fileIndex} 
                            className={`overflow-hidden rounded-lg ${item.post_files.length === 1 ? 'max-h-96' : 'h-48'} bg-gray-100`}
                          >
                            <img 
                    ...
                    {!item?.post_files && item?.files && item.files.length > 0 && (
                    ...
                    {!item?.post_files && !item?.files && item?.file && (
                      <div className="mt-3">
                        <div className="overflow-hidden rounded-lg max-h-96 bg-gray-100">
                          <img 
                            src={typeof item.file === 'string' ? item.file : (item.file.url || item.file.path || '/placeholder-image.jpg')} 
                            alt="Post image" 
                            className="w-full h-full object-cover"
                          />
                        </div>
